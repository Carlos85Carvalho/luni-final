      ...
      <div className="w-full max-w-7xl mx-auto px-4 pt-6 md:px-8 md:pt-10 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <button onClick={() => { setAgendamentoParaEditar(null); setModalTipo('agendamento'); setIsModalOpen(true); }} className="flex-1 md:flex-none bg-gradient-to-r from-[#5B2EFF] to-[#7C3EFF] px-6 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="flex bg-[#1c1c24] p-1.5 rounded-2xl border border-white/10 w-full md:w-auto overflow-x-auto">
            {[
              ...
              const Icon = v.icon;
              return (
                <button key={v.value} onClick={() => setVisualizacao(v.value)} className={`flex-1 md:flex-none px-4 py-2.5 rounded-xl text-[11px] font-bold flex items-center justify-center gap-2 whitespace-nowrap transition-all ${visualizacao === v.value ? 'bg-[#5B2EFF] text-white shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}>
          ...
          <div className="relative flex-1">
            ...
            <input type="text" value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar cliente..." className="w-full bg-[#1c1c24] border border-white/10 rounded-2xl pl-12 pr-12 py-3.5 text-white outline-none focus:border-purple-500/50"/>
          ...
          <button onClick={() => setShowFiltros(!showFiltros)} className={`px-4 py-3.5 rounded-2xl font-bold flex items-center gap-2 border ${showFiltros || filtroStatus !== 'todos' ? 'bg-purple-500/20 text-purple-300 border-purple-500/30' : 'bg-[#1c1c24] text-gray-400 border-white/10'}`}>
            <Filter size={18}/> <ChevronDown size={16} className={`transition-transform ${showFiltros ? 'rotate-180' : ''}`}/>
          </button>
        </div>

        {showFiltros && (
          <div className="bg-[#1c1c24] border border-white/10 rounded-2xl p-4 mb-6">
            ...
            <div className="flex flex-wrap gap-2">
              {['todos', 'agendado', 'confirmado', 'concluido', 'bloqueado', 'cancelado'].map(status => (
                <button key={status} onClick={() => setFiltroStatus(status)} className={`px-4 py-2 rounded-xl text-xs font-bold capitalize border ${filtroStatus === status ? 'bg-[#5B2EFF] text-white border-[#5B2EFF]' : 'bg-white/5 text-gray-400 border-transparent'}`}>{status}</button>
              ))}
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? (
            <div className="col-span-full py-20 text-center"><Loader2 className="animate-spin mx-auto text-purple-500 mb-2" size={32}/><p className="text-gray-500 text-sm">Carregando...</p></div>
          ) : agendamentosFiltrados.length === 0 ? (
            <div className="col-span-full py-12 flex flex-col items-center justify-center text-center border-2 border-dashed border-white/10 rounded-3xl bg-white/5">
              <CalendarDays size={40} className="text-gray-600 mb-3"/><p className="text-gray-400 font-medium">Nenhum agendamento</p>
            </div>
          ) : (
              ...
              <div key={item.id} className={`bg-[#15151a] relative rounded-3xl p-5 border shadow-lg hover:border-purple-500/30 transition-all group ${item.status === 'bloqueado' ? 'border-orange-500/20' : item.status === 'concluido' ? 'border-white/10 bg-white/5 opacity-70' : 'border-white/5'}`}>
                ...
                <div className="flex justify-between items-start mb-4">
                  <div className="flex gap-4 w-full">
                    {/* Data/Hora Box */}
                    <div className="bg-[#1c1c24] px-3 py-2 rounded-2xl text-center border border-white/5 min-w-[70px] h-fit">
                      <span className={`text-[10px] block font-black uppercase mb-0.5 ${item.status === 'bloqueado' ? 'text-orange-500' : 'text-purple-400'}`}>{formatDataCard(item.data)}</span>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.profissionais?.nome && (
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span className="text-[10px] text-purple-300 font-bold uppercase truncate">{item.profissionais.nome}</span>
                        </div>
                      )}
                  ...
                  <div className="flex flex-col items-end gap-2">
                    {item.status !== 'cancelado' && (
                      <div className="relative">
                        <button onClick={(e) => { e.stopPropagation(); setMenuAberto(menuAberto === item.id ? null : item.id); }} className="p-2 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white transition-colors"><MoreVertical size={18}/></button>
                        {menuAberto === item.id && (
