...
export const RemarcarModal = ({ isOpen, onClose, onSuccess, agendamento }) => {
  ...
  useEffect(() => {
    if (isOpen && agendamento) {
      // Puxa o profissional que já estava selecionado no agendamento atual
      setSelectedProfissional(agendamento.profissional_id || '');
      setData(agendamento.data || '');
      setHorario(agendamento.horario || '');

      // Busca a lista de profissionais para permitir a troca se necessário
  ...
  const handleUpdate = async () => {
    ...
    setLoading(true);
    try {
      // Atualiza no Supabase mantendo a consistência dos campos de valor que você criou
      const { error } = await supabase
        ...
        .update({
          ...
          status: 'agendado' // Ao remarcar, o status volta para agendado para reativar o fluxo
        })
      ...
      setTimeout(() => {
        ...
        onSuccess(); // Atualiza a lista da agenda
        onClose();
      }, 1000);
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-end sm:items-center justify-center bg-black/80 backdrop-blur-sm p-0 sm:p-4 animate-in fade-in duration-200">
      <div className="bg-[#18181b] w-full sm:max-w-sm sm:rounded-3xl rounded-t-3xl border border-white/10 shadow-2xl relative animate-in slide-in-from-bottom-10 duration-300">
