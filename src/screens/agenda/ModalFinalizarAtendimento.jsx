...
export const ModalFinalizarAtendimento = ({ isOpen, onClose, agendamento, onSuccess }) => {
  ...
  // 1. HOOKS NO TOPO (Sempre antes de qualquer return)
  const [loading, setLoading] = useState(false);
  // Proteção para não quebrar se agendamento for nulo
  ...
  // useEffect também fica aqui no topo
  useEffect(() => {
  ...
  // 2. AGORA SIM O RETURN DE CONTROLE (GUARD CLAUSE)
  // Se colocar isso antes dos Hooks acima, o código quebra
  ...
  const handleFinalizar = async () => {
    ...
    try {
      ...
      // Aqui corrigimos o erro 'unused var' usando o erro
