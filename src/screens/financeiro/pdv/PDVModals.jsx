      ...
      <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl animate-in zoom-in-95">
        <div className="flex justify-between items-center mb-6">
          <div>
            ...
            <p className="text-gray-500 text-sm">Total a pagar: <span className="font-bold text-gray-900">R$ {total?.toFixed(2)}</span></p>
          </div>
...
// --- MODAL DE SERVIÇOS E AGENDAMENTOS ---
export const ServicosModal = ({ aberto, onClose, agendamentos = [], servicos = [], onAdicionar }) => {
  if (!aberto) return null;
  return (
    <div className="fixed inset-0 bg-black/50 z-[70] flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white rounded-2xl w-full max-w-4xl h-[80vh] flex flex-col shadow-2xl animate-in zoom-in-95">
        <div className="p-4 border-b flex justify-between items-center bg-gray-50 rounded-t-2xl">
          <h3 className="font-bold text-lg text-gray-800 flex items-center gap-2"><Scissors className="text-purple-600" /> Serviços & Agendamentos</h3>
          <button onClick={onClose}><X size={20} className="text-gray-500 hover:text-gray-800"/></button>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {/* Agendamentos */}
          <div className="mb-8">
            <h4 className="font-bold text-gray-700 mb-3 text-sm uppercase tracking-wider">Agendamentos de Hoje</h4>
            {agendamentos.length === 0 ? (
              <p className="text-gray-400 text-sm italic">Nenhum agendamento confirmado para hoje.</p>
            ) : (
              <div className="grid md:grid-cols-2 gap-3">
                {agendamentos.map(ag => (
                  <div key={ag.id} className="bg-blue-50 p-4 rounded-xl border border-blue-100 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start">
                      <div>
                        <div className="font-bold text-blue-900">{ag.servicos?.nome}</div>
                        <div className="text-sm text-blue-700">{ag.clientes?.nome}</div>
                        <div className="text-xs text-blue-500 mt-1">{ag.hora_inicio}</div>
                      </div>
                      <button onClick={() => onAdicionar(ag, 'agendamento')} className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-colors">
                        Adicionar
                      </button>
                    </div>
                  </div>
          ...
          {/* Lista de Serviços */}
          <div>
            <h4 className="font-bold text-gray-700 mb-3 text-sm uppercase tracking-wider">Todos os Serviços</h4>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {servicos.map(serv => (
                <button key={serv.id} onClick={() => onAdicionar(serv, 'servico')} className="group bg-white p-3 rounded-xl border border-gray-200 hover:border-purple-500 hover:shadow-md transition-all text-left">
                  <div className="font-semibold text-gray-800 group-hover:text-purple-700 truncate">{serv.nome}</div>
                  <div className="flex justify-between mt-2 text-sm">
                    <span className="text-gray-500 text-xs">{serv.duracao} min</span>
                    <span className="font-bold text-green-600">R$ {serv.preco_base?.toFixed(2)}</span>
                  </div>
                </button>
              ))}
            </div>
                ...
                <div key={venda.id} className="bg-white p-4 rounded-xl border border-amber-100 shadow-sm">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="text-right">
                      <div className="font-bold text-lg">R$ {venda.total?.toFixed(2)}</div>
