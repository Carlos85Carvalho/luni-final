    ...
    try {
      ...
      await Promise.all(carrinho.map(async (item) => {
        if (item.tipo === 'produto') {
          ...
          // Ajuste aqui se sua tabela usa apenas 'estoque' ou ambos
          return Promise.all([
            supabase.from('produtos').update({ estoque: novoEstoque }).eq('id', item.id),
      ...
      // AQUI: Passando os dados completos para o modal gerar o PDF
      setModalState({ 
        view: 'sucesso', 
        isOpen: true,
        dados: { 
