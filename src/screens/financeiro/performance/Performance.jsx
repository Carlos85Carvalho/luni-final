...
export const Performance = () => {
  ...
  return (
    <div className={`min-h-full ${THEME.bg} p-4 md:p-8 space-y-6 animate-in fade-in duration-500`}>
      {/* HEADER */}
      <div className={`${THEME.card} p-4 md:p-6 rounded-3xl border ${THEME.border} flex flex-col gap-4 md:gap-6 shadow-xl`}>
          ...
          <div className={`bg-[#0f0f12] p-1 rounded-xl border border-white/10 flex overflow-x-auto scrollbar-hide`}>
            {['hoje', 'semana', 'mes', 'ano'].map((p) => (
              <button
                ...
                className={`flex-1 min-w-[70px] px-3 py-2 text-xs font-bold rounded-lg transition-all capitalize whitespace-nowrap ${
