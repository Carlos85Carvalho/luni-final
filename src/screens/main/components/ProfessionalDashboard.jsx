// src/screens/main/ProfessionalDashboard.jsx
...
// Componentes Visuais (Reutilizando do Admin)
...
import { 
  ...
  Wallet, Scissors, Star, Bell, MessageCircle, Plus, Lock, Search, X, Check, MapPin
} from 'lucide-react';
...
export const ProfessionalDashboard = ({ profissional, onLogout }) => {
  ...
  const fetchDados = async () => {
    ...
    // Tenta pegar valor_total ou valor
  ...
  const handleAgendamentoSucesso = () => { setIsModalOpen(false); fetchDados(); };

  const getProximoCliente = () => {
    const agora = new Date();
    // Filtra agendamentos de HOJE que ainda vão acontecer
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-white w-full overflow-x-hidden">
      ...
      <div className="w-full max-w-7xl mx-auto px-4 pt-6 md:px-8 md:pt-10 pb-24 space-y-8 animate-in fade-in duration-700">
        ...
        {/* CONTROLES PRINCIPAIS (Abas e Busca) */}
        ...
        {tab === 'agenda' && (
           <div className="space-y-6">
              ...
              {/* CALENDÁRIO HORIZONTAL (Só aparece se não estiver buscando) */}
              {!busca && (
        ...
        {/* CONTEÚDO DA ABA FINANCEIRO (Exemplo Simples) */}
        {tab === 'financeiro' && (
